...
export function ClassLoadView({
  ...
  const [selectedTeacher, setSelectedTeacher] =
    React.useState<TeacherLoadSummary | null>(null);
  ...
  const loadList = React.useMemo(() => {
    return Object.values(teacherLoads).sort(
      (a, b) => b.totalWeekLoad - a.totalWeekLoad,
    );
  }, [teacherLoads]);
  ...
  const avgLoadPerTeacher = (
    totalPeriodsWeek / Math.max(1, loadList.length)
  ).toFixed(1);
  const overloadedCount = loadList.filter(
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <div className="p-4 sm:p-5 rounded-2xl bg-card border border-border shadow-xs">
          ...
          <p className="text-[10px] sm:text-[11px] text-foreground-muted mt-0.5">
            Active Staff
          </p>
        </div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-3 p-3.5 sm:p-4 rounded-2xl bg-card border border-border shadow-xs">
        <div className="relative w-full sm:w-72">
          ...
          <input
            type="text"
            placeholder="Search teacher acronym, dept..."
            value={searchTerm}
          ...
          {[
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl font-medium transition-all whitespace-nowrap cursor-pointer shrink-0 ${
                  isSelected
        ...
        {teachers && onAddTeacher && onRemoveTeacher && onEditTeacher && (
          <button
            ...
            className="w-full sm:w-auto justify-center inline-flex items-center gap-1.5 px-3 py-2 sm:py-1.5 text-xs font-semibold rounded-xl bg-primary text-primary-foreground hover:bg-primary/90 transition-colors shadow-xs shrink-0 cursor-pointer"
          >
      ...
      <div className="rounded-3xl bg-card border border-border shadow-sm overflow-hidden">
        <div className="sm:hidden flex items-center justify-between text-[11px] text-foreground-muted px-4 py-2 border-b border-border">
          <span>Swipe horizontally for days breakdown</span>
          ...
          <table className="w-full text-left text-xs border-collapse">
            ...
            <tbody className="divide-y divide-border/60">
              {filteredList.map((item) => (
                ...
                >
                  <td className="py-3 px-4 font-mono font-bold text-foreground sticky left-0 z-10 bg-card">
          ...
          <div className="bg-card border border-border p-6 rounded-3xl shadow-2xl max-w-lg w-full space-y-4">
            <div className="flex items-center justify-between border-b border-border pb-3">
              <div>
                ...
                <p className="text-xs text-foreground-muted">
                  Department: {selectedTeacher.teacher.dept} • Weekly Load:{" "}
                  {selectedTeacher.totalWeekLoad} periods
                </p>
              </div>
            ...
            <div className="space-y-3 text-xs">
              ...
              {["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday"].map(
                (dayName) => {
                  const freeSlots =
                    selectedTeacher.freeSlotsByDay[dayName] || [];
